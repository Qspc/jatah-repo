...
export default function PengaturanAsrama({ id = "1" }: props) {
    ...
    const { setIsHaveAsrama, kelompok } = usePengaturan();
    const { data: allAsrama, isLoading } = useQuery({
        queryKey: ["asrama"],
        queryFn: async () => {
            const data = await getAllAsrama(Number(id));
            if (data.length > 0) setIsHaveAsrama(true);
    ...
    return (
        <div className="flex flex-col gap-2">
            <ul className="space-y-1 text-gray-800 list-disc list-inside">
                <li className="font-semibold capitalize text-[24px]">
                    {kelompok}
                </li>
            </ul>
            <div>
                <AsramaDialog
                    ...
                    buttonTrigger={
                        <div className="flex items-center gap-2 capitalize button-primary ">
                            {isLoading && <LoadingButton />} {kelompok} Baru +
                        </div>
                    }
                />
            </div>
